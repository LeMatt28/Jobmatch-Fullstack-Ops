...
router.get("/me/matches", verifyToken, async (req: Request, res: Response) => {
    try {
        ...
        const matches = await prisma.match.findMany({ where: { candidateId }, include: { offer: { include: { company: { select: { id: true, name: true, scoreReliability: true } } } } } });
...
router.get("/matches/:offerId", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    const matches = await prisma.match.findMany({
      ...
      include: {
        candidate: {
          select: { id: true, name: true, skills: true, softSkills: true, experience: true, salaryExpected: true, scoreCandidat: true },
        },
      },
    });
